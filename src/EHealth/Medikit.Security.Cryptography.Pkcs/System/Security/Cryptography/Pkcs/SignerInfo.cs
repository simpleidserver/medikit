...
using Medikit.Security.Cryptography.Xml;
using System;
...
namespace Medikit.Security.Cryptography.Pkcs
{
    public sealed class SignerInfo
    {
        ...
        public ReadOnlyMemory<byte>? SignatureAlgorithmParameters
        {
            get
            {
                return _signatureAlgorithmParameters;
            }
        }
        ...
        private void WithSelfInfo(WithSelfInfoDelegate action)
        {
            if (_parentSignerInfo == null)
            {
                ...
                if (myIdx < 0)
                {
                    throw new CryptographicException(Strings.Cryptography_Cms_SignerNotFound);
                }
            ...
            else
            {
                ...
                if (parentIdx == -1)
                {
                    throw new CryptographicException(Strings.Cryptography_Cms_NoSignerAtIndex);
                }
        ...
        private static void RemoveUnsignedAttribute(ref SignerInfoAsn mySigner, AsnEncodedData unsignedAttribute)
        {
            ...
            {
                throw new CryptographicException(Strings.Cryptography_Cms_NoAttributeFound);
            }
        ...
        {
            if (_parentSignerInfo != null)
                throw new CryptographicException(Strings.Cryptography_Cms_NoCounterCounterSigner);
            if (signer == null)
        ...
        public void CheckHash()
        {
            ...
            {
                throw new CryptographicException(Strings.Cryptography_Pkcs_InvalidSignatureParameters);
            }

            if (!CheckHash(compatMode: false) && !CheckHash(compatMode: true))
            {
                throw new CryptographicException(Strings.Cryptography_BadSignature);
            }
        }
        ...
        private IncrementalHash? PrepareDigest(bool compatMode)
        {
            ...
            if (invalid)
            {
                throw new CryptographicException(Strings.Cryptography_Cms_MissingAuthenticatedAttribute);
            }
            ...
            bool verifySignatureOnly)
        {
            ...
            {
                throw new CryptographicException(Strings.Cryptography_Cms_UnknownAlgorithm, SignatureAlgorithm.Value);
            }
            ...
            if (!verifySignatureOnly)
            {
                ...
                if (!chain.Build(certificate))
                {
                    ...
                    throw new CryptographicException(Strings.Cryptography_Cms_TrustFailure, status.StatusInformation);
                }
                ...
                foreach (X509Extension ext in certificate.Extensions)
                {
                    if (ext.Oid!.Value == Oids.KeyUsage)
                    {
                        ...
                        {
                            throw new CryptographicException(Strings.Cryptography_Cms_WrongKeyUsage);
                        }
                    }
                }
            }
